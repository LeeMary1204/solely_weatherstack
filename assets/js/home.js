...
var favicon = new Favico({ animation : 'popFade', position : 'up' });
...
function callMonitorList(apiPageValue, initialLoad) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            // Latest downtime
            if (data.statistics.latest_downtime !== null && window.showO) {
            ...
            // Update event feed if it is initial load or there was a change in status...
            if (initialLoad) {
...
function sortAnnouncements() {
    ...
    $('.announcement-feed .psp-announcement').removeClass('is-last');
    if ($('.announcement-feed .psp-announcement').length > 0) {
        var sorted = $('.announcement-feed').find('.psp-announcement').sort(function(a, b) {
           return $(b).attr('data-time') - $(a).attr('data-time');
        });

        $('.announcement-feed').html(sorted);

        if ($('.announcement-feed .psp-announcement').length > 5) {
            $.each($('.announcement-feed .psp-announcement'), function(i) {
                if (i > 4) {
                    $(this).addClass('uk-hidden');

                }
            });
            $('.announcement-feed .psp-announcement:visible:last').addClass('is-last');
            $('.load-more-ann-f').parent().removeClass('uk-hidden');
            $('.announcement-feed-end').addClass('uk-hidden');
            $('.load-more-ann').parent().addClass('uk-hidden');
        }
    } else {
        $('.announcement-feed').addClass('uk-hidden');
        $('.announcement-feed-end').removeClass('uk-hidden').find('> em').text('There are no status updates in the last 3 months.');
    }
}
*/
